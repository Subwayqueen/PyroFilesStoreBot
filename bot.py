...
async def start(bot, cmd):
	...
	if usr_cmd == "/start":
				...
				await bot.send_message(
					...
					reply_markup=InlineKeyboardMarkup(
						[
							...
							[
								InlineKeyboardButton("🔄 Refresh / Try Again", url=f"https://telegram.dog/{BOT_USERNAME}?start=view_{file_id}_tg")
							]
						]
					),
					parse_mode="markdown"
		...
		try:
			...
			await send_stored_file.reply_text(f"**Link To Your File Is Below :**\n\nhttps://telegram.dog/{BOT_USERNAME}?start=view_{file_id}_tg\n\n**To Retrive The Stored File, Just Open The Link! 👍**", disable_web_page_preview=True, quote=True)
...
async def main(bot, message):
	if message.chat.type == "private":
		...
		try:
			...
			share_link = f"https://telegram.dog/{BOT_USERNAME}?start=view_{file_er_id}_tg"
			await editable.edit(
